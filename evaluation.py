...
class kittiOdomEval():
    ...
    def loadPosestxt(self, file_name, toCameraCoord):
        '''
            Each line in the file should follow one of the following structures
            (1) idx pose(3x4 matrix in terms of 12 numbers)
            (2) pose(3x4 matrix in terms of 12 numbers)
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            poses_gt = self.loadPosestxt(gt_file_name, toCameraCoord=False)
            # print(poses_gt)
            ...
            with open(save_txt, 'a+') as tt:
                ...
                tt.write('Average sequence rotational error(deg/cm): {0:.4f} \n'.format(ave_r_err / np.pi * 180*100))

            print("Average sequence rotational error (deg/cm): {0:.4f}\n".format(ave_r_err / np.pi * 180*100))
