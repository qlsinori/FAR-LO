...
def PreProcess(PC_f1, PC_f2, T_gt):    ####    pre process procedure
    ...
    for i in range(batch_size):
        ...
        cur_PC_f1_concat = torch.where( r_f1 > 100 , torch.zeros_like(cur_PC_f1_concat).cuda(), cur_PC_f1_concat ).to(torch.float32)
        ...
        cur_PC_f2_concat = torch.where( r_f2 > 100 , torch.zeros_like(cur_PC_f2_concat).cuda(), cur_PC_f2_concat ).to(torch.float32)
