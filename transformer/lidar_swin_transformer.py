...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size_w=8, window_size_h=4, window_size=7, shift_size_w = 4,shift_size_h = 2,shift_size=0,
    ...
    def forward(self, x, mask_in):
        ...
        # cyclic shift
        if self.shift_size_h or self.shift_size_w > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size_h, -self.shift_size_w), dims=(1, 2))
            shifted_mask = torch.roll(mask_in, shifts=(-self.shift_size_h, -self.shift_size_w), dims=(1, 2))
        else:
            shifted_x = x
        ...
        mask_windows = window_partition(shifted_mask, self.window_size_h,self.window_size_w)#(num_windows*Batch, window_size, window_size, 1)
        x_windows = x_windows.view(-1, self.window_size_h * self.window_size_w, C)  # nW*B, window_size*window_size, C
        mask_windows = mask_windows.view(-1, self.window_size_h * self.window_size_w, 1)  # nW*B, window_size*window_size, 1
...
class lidar_BasicLayer(nn.Module):
