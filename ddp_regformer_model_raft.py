...
from transformer.lidar_cross_swin_transformer import lidar_Cross_BasicLayer
...
class regformer_model(nn.Module):
    def __init__(self, args, batch_size, H_input, W_input, is_training,show_time,multi_gpu,writer, bn_decay=None):
        ...
        self.num_iter = 2 #2
        self.skip = True
        self.cost_list_volume2 = nn.ModuleList(
            [cost_volume(batch_size=batch_size, kernel_size1=[3, 5], kernel_size2=[5, 15], nsample=4,
                                        nsample_q=6, \
                                        H=self.out_H_list[2], W=self.out_W_list[2], \
                                        stride_H=1, stride_W=1, distance=self.Cost_volume_dis[2],
                                        in_channels=[64, 64],
                                        mlp1=[128, 64, 64], mlp2=[128, 64], is_training=self.training,
                                        bn_decay=bn_decay,
                                        bn=True, pooling='max', knn=True, corr_func='concat') for _ in range(self.num_iter)])

        self.cost_list_volume3 = nn.ModuleList(
            [ cost_volume(batch_size=batch_size, kernel_size1=[3, 5], kernel_size2=[7, 25], nsample=4,
                                        nsample_q=6, \
                                        H=self.out_H_list[1], W=self.out_W_list[1], \
                                        stride_H=1, stride_W=1, distance=self.Cost_volume_dis[1],
                                        in_channels=[32, 32],
        ...
        self.cost_list_volume4 = nn.ModuleList(
            [ cost_volume(batch_size=batch_size, kernel_size1=[3, 5], kernel_size2=[11, 41], nsample=4,
                                        nsample_q=6, \
                                        H=self.out_H_list[0], W=self.out_W_list[0], \
                                        stride_H=1, stride_W=1, distance=self.Cost_volume_dis[0],
                                        in_channels=[16, 16],
        ...
        self.flow_predictor1_list_predict = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 3, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        self.flow_predictor1_list_w = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 3, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        self.flow_predictor2_list_predict = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 2 + 32, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        self.flow_predictor2_list_w = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 2 + 32, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        self.flow_predictor3_list_predict = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 2 + 16, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        self.flow_predictor3_list_w = nn.ModuleList(
            [FlowPredictor(in_channels=64 * 2 + 16, mlp=[128, 64], is_training=self.training,
                           bn_decay=bn_decay) for _ in range(self.num_iter)])
        ...
        self.set_upconv3_upsample = set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                      ...
                                                      bn_decay=bn_decay, knn=True)
###################list#################################
        self.set_upconv1_list_w_upsample =  nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                        H=self.out_H_list[2], W=self.out_W_list[2],
                                                        stride_H=self.stride_H_list[-1],
                                                        stride_W=self.stride_W_list[-1],
                                                        nsample=8, distance=self.Up_conv_dis[2],
        ...
        self.set_upconv1_list_upsample = nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                      H=self.out_H_list[2], W=self.out_W_list[2],
                                                      stride_H=self.stride_H_list[-1], stride_W=self.stride_W_list[-1],
                                                      nsample=8, distance=self.Up_conv_dis[2],
                                                      in_channels=[64, 64],
        ...
        self.set_upconv2_list_w_upsample = nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                        H=self.out_H_list[1], W=self.out_W_list[1],
                                                        stride_H=self.stride_H_list[-2],
                                                        stride_W=self.stride_W_list[-2], \
                                                        nsample=8, distance=self.Up_conv_dis[1],
        ...
        self.set_upconv2_list_upsample = nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                      H=self.out_H_list[1], W=self.out_W_list[1],
                                                      stride_H=self.stride_H_list[-2], stride_W=self.stride_W_list[-2], \
                                                      nsample=8, distance=self.Up_conv_dis[1],
                                                      in_channels=[32, 64],
        ...
        self.set_upconv3_list_w_upsample = nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                        H=self.out_H_list[0], W=self.out_W_list[0],
                                                        stride_H=self.stride_H_list[-3],
                                                        stride_W=self.stride_W_list[-3], \
                                                        nsample=8, distance=self.Up_conv_dis[0],
        ...
        self.set_upconv3_list_upsample = nn.ModuleList([set_upconv_module(batch_size=batch_size, kernel_size=[7, 15],
                                                      H=self.out_H_list[0], W=self.out_W_list[0],
                                                      stride_H=self.stride_H_list[-3], stride_W=self.stride_W_list[-3], \
                                                      nsample=8, distance=self.Up_conv_dis[0],
                                                      in_channels=[16, 64],
        ...
        self.conv1_list_l2 = nn.ModuleList([Conv1d(256, 4, use_activation=False) for _ in range(self.num_iter)])
        self.conv1_list_l1 = nn.ModuleList([Conv1d(256, 4, use_activation=False) for _ in range(self.num_iter)])
        self.conv1_list_l0 = nn.ModuleList([Conv1d(256, 4, use_activation=False) for _ in range(self.num_iter)])

        self.conv2_list_l2 = nn.ModuleList([Conv1d(256, 3, use_activation=False) for _ in range(self.num_iter)])
        self.conv2_list_l1 = nn.ModuleList([Conv1d(256, 3, use_activation=False) for _ in range(self.num_iter)])
        self.conv2_list_l0 = nn.ModuleList([Conv1d(256, 3, use_activation=False) for _ in range(self.num_iter)])

        self.conv3_list_l2 = nn.ModuleList([Conv1d(64, 256, use_activation=False) for _ in range(self.num_iter)])
        self.conv3_list_l1 = nn.ModuleList([Conv1d(64, 256, use_activation=False) for _ in range(self.num_iter)])
        self.conv3_list_l0 = nn.ModuleList([Conv1d(64, 256, use_activation=False) for _ in range(self.num_iter)])
...
#                                 drop_path=0.1, norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False)
        ...
        self.cross_trans2 = lidar_Cross_BasicLayer(dim=64, input_resolution=(self.out_H_list[2], self.out_W_list[2]),
                                           depth=6, num_heads=8,
                                           window_size=[[4, 4], [4, 4], [4, 4], [4, 4], [4, 8], [4, 8], [4, 8],
                                                        [4, 8], [4, 8]],
                                           shift_size=[[0, 0], [2, 2], [0, 0], [2, 2], [0, 0], [2, 4], [0, 0],
                                                       [0, 4], [0, 0]],
                                           mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.cnet = BasicEncoder(output_dim=128 + 128, norm_fn='none', dropout=0.0)
        # self.update_block = BasicUpdateBlock( hidden_dim=128)

        self.loftr = LocalFeatureTransformer(d_model=64, nhead=8, layer_names=['self', 'cross'])
    ...
    def forward(self, input_xyz_f1, input_xyz_f2, T_gt, T_trans, T_trans_inv):
        ...
        with torch.no_grad():
            ...
            if self.multi_gpu is not None:
                input_xyz_f1 = torch.unbind(input_xyz_f1, dim=0)
                input_xyz_f1 = list(input_xyz_f1)
                input_xyz_f2 = torch.unbind(input_xyz_f2, dim=0)
                input_xyz_f2 = list(input_xyz_f2)
            ...
            # l0_points_f1 = self.swin0(l0_points_f1)
            ...
            # l0_points_f2 = self.swin0(l0_points_f2)
            ...
            l2_mask_f1_2d = torch.reshape(l2_mask_f1, (batch_size,  -1))
            l2_mask_f2_2d = torch.reshape(l2_mask_f2, (batch_size,  -1))
            l2_points_f1_loftr,l2_points_f2_loftr = self.loftr(l2_points_f1,l2_points_f2,l2_mask_f1_2d,l2_mask_f2_2d)

            l2_points_cross_f1, l2_points_cross_f2 = self.cross_trans2(l2_points_f1_loftr, l2_points_f2_loftr, l2_mask_f1, l2_mask_f2)
            ...
            l2_points_cross_proj_f1 = torch.reshape(l2_points_cross_f1,
                                                    [batch_size, self.out_H_list[2], self.out_W_list[2], -1])
            l2_points_cross_proj_f2 = torch.reshape(l2_points_cross_f2,
                                                    [batch_size, self.out_H_list[2], self.out_W_list[2], -1])
    ...
    ########################################################
            pre_qt = []
            pre_qt.append([l3_q_norm,l3_t])
            q_now = l3_q_norm
            t_now = l3_t

        for itr in range(self.num_iter + 1):

            ######## l2 pwc
            if itr < 1 or not(self.skip):
                l2_q_coarse = torch.reshape(q_now, [batch_size, 1, -1])
                l2_t_coarse = torch.reshape(t_now, [batch_size, 1, -1])
                l2_q_inv = inv_q(l2_q_coarse, batch_size)

                ### warp layer2 pose
                l2_xyz_f1 = torch.reshape(l2_xyz_proj_f1, [batch_size, -1, 3]).clone()
                ...
                ### re-project
                l2_xyz_warp_proj_f1, l2_points_warp_proj_f1 = ProjectPCimg2SphericalRing(l2_flow_warped, l2_points_f1,
                                                                                         self.out_H_list[2],
                                                                                         self.out_W_list[2])
                ...
                l2_mask_warped_proj = torch.reshape(l2_mask_warped,
                                                    [batch_size, self.out_H_list[2], self.out_W_list[2], -1])

                # get the cost volume of warped layer2 flow and the points of frame2

                l2_points_warp_cross_f1, l2_points_warp_cross_f2 = self.cross_trans2(l2_points_warp_f1, l2_points_f2,
                                                                                     l2_mask_warped_proj, l2_mask_f2)
                l2_points_warp_cross_proj_f1 = torch.reshape(l2_points_warp_cross_f1,
                                                             [batch_size, self.out_H_list[2], self.out_W_list[2], -1])
                l2_points_warp_cross_proj_f2 = torch.reshape(l2_points_warp_cross_f2,
                                                             [batch_size, self.out_H_list[2], self.out_W_list[2], -1])
                if itr == 0:
                    l2_cost_volume = self.cost_volume2(l2_xyz_warp_proj_f1, l2_xyz_proj_f2, l2_points_warp_cross_proj_f1,
                                                   l2_points_warp_cross_proj_f2)  # FE2
                else:
                    l2_cost_volume = self.cost_list_volume2[itr - 1](l2_xyz_warp_proj_f1, l2_xyz_proj_f2, l2_points_warp_cross_proj_f1,
                                                   l2_points_warp_cross_proj_f2)
                # l2_cost_volume = self.cost_volume2(l2_xyz_warp_proj_f1, l2_xyz_proj_f2, l2_points_warp_cross_proj_f1,
                #                                    l2_points_warp_cross_proj_f2) 
                if itr == 0:
                    ...
                    l2_cost_volume_w_upsample = self.set_upconv1_list_w_upsample[itr - 1](l2_xyz_warp_proj_f1, l3_xyz_proj_f1,
                                                                            l2_points_warp_proj_f1, l3_cost_volume_w_proj)
                if itr == 0:
                    l2_cost_volume_upsample = self.set_upconv1_upsample(l2_xyz_warp_proj_f1, l3_xyz_proj_f1,
                                                                  l2_points_warp_proj_f1, l3_cost_volume_proj)
                else:
                    l2_cost_volume_upsample = self.set_upconv1_list_upsample[itr - 1](l2_xyz_warp_proj_f1, l3_xyz_proj_f1,
                                                                  l2_points_warp_proj_f1, l3_cost_volume_proj)
                ...
                if itr == 0:
                    ...
                    l2_cost_volume_predict = self.flow_predictor1_list_predict[itr - 1](l2_points_warp_f1, l2_cost_volume_upsample,
                                                                          l2_cost_volume)
                if itr == 0:
                    l2_cost_volume_w = self.flow_predictor1_w(l2_points_warp_f1, l2_cost_volume_w_upsample,
                                                              l2_cost_volume_predict)
                else:
                    l2_cost_volume_w = self.flow_predictor1_list_w[itr - 1](l2_points_warp_f1, l2_cost_volume_w_upsample,
                                                              l2_cost_volume_predict)
                ...
                if itr == 0 :
                    ...
                    l2_points_f1_new_big = self.conv3_list_l2[itr-1](l2_cost_volume_sum)
                    # l2_points_f1_new_big = self.conv3_2(l2_cost_volume_sum)
                ...
                if itr == 0:
                    ...
                    l2_q_det = self.conv1_list_l2[itr - 1](l2_points_f1_new_q)
                    # l2_q_det = self.conv1_2(l2_points_f1_new_q)
                ...
                if itr == 0:
                    ...
                    l2_t_det = self.conv2_list_l2[itr - 1](l2_points_f1_new_t)
                    # l2_t_det = self.conv2_2(l2_points_f1_new_t)
                ...
                q_now = l2_q_norm
                t_now = l2_t
                pre_qt.append([l2_q_norm,l2_t])

            ############# layer1 PWC ################

            if itr < 2 or not(self.skip):
                l1_q_coarse = torch.reshape(q_now, [batch_size, 1, -1])
                l1_t_coarse = torch.reshape(t_now, [batch_size, 1, -1])
                l1_q_inv = inv_q(l1_q_coarse, batch_size)

                ############# warp layer1 pose
                l1_xyz_f1 = torch.reshape(l1_xyz_proj_f1, [batch_size, -1, 3]).clone()
                ...
                ########## re-project
                l1_xyz_warp_proj_f1, l1_points_warp_proj_f1 = ProjectPCimg2SphericalRing(l1_flow_warped, l1_points_f1,
                                                                                         self.out_H_list[1],
                                                                                         self.out_W_list[1])  #
                ...
                l1_mask_warped_proj = torch.reshape(l1_mask_warped,
                                                    [batch_size, self.out_H_list[1], self.out_W_list[1], -1])

                # get the cost volume of warped layer1 flow and the points of frame2
                l1_points_warp_cross_f1, l1_points_warp_cross_f2 = self.cross_trans1(l1_points_warp_f1, l1_points_f2,
                                                                                     l1_mask_warped_proj, l1_mask_f2)
                l1_points_warp_cross_proj_f1 = torch.reshape(l1_points_warp_cross_f1,
                                                             [batch_size, self.out_H_list[1], self.out_W_list[1], -1])
                l1_points_warp_cross_proj_f2 = torch.reshape(l1_points_warp_cross_f2,
                                                             [batch_size, self.out_H_list[1], self.out_W_list[1], -1])
                if itr ==0:
                    l1_cost_volume = self.cost_volume3(l1_xyz_warp_proj_f1, l1_xyz_proj_f2, l1_points_warp_cross_proj_f1,
                                                   l1_points_warp_cross_proj_f2)  # FE1
                else:
                    l1_cost_volume = self.cost_list_volume3[itr - 1](l1_xyz_warp_proj_f1, l1_xyz_proj_f2, l1_points_warp_cross_proj_f1,
                                                   l1_points_warp_cross_proj_f2)  # FE1
                # l1_cost_volume = self.cost_volume3(l1_xyz_warp_proj_f1, l1_xyz_proj_f2, l1_points_warp_cross_proj_f1,
                #                                    l1_points_warp_cross_proj_f2)
                if itr ==0:
                    ...
                    l1_cost_volume_w_upsample = self.set_upconv2_list_w_upsample[itr - 1](l1_xyz_warp_proj_f1, l2_xyz_warp_proj_f1,
                                                                            l1_points_warp_proj_f1, l2_cost_volume_w_proj)
                if itr == 0:
                    ...
                    l1_cost_volume_upsample = self.set_upconv2_list_upsample[itr - 1](l1_xyz_warp_proj_f1, l2_xyz_warp_proj_f1,
                                                                             l1_points_warp_proj_f1, l2_cost_volume_proj)
                ...
                if itr == 0:
                    ...
                    l1_cost_volume_predict = self.flow_predictor2_list_predict[itr - 1](l1_points_warp_f1, l1_cost_volume_upsample,
                                                                          l1_cost_volume)
                if itr == 0:
                    l1_cost_volume_w = self.flow_predictor2_w(l1_points_warp_f1, l1_cost_volume_w_upsample,
                                                          l1_cost_volume_predict)
                else:
                    l1_cost_volume_w = self.flow_predictor2_list_w[itr - 1](l1_points_warp_f1, l1_cost_volume_w_upsample,
                                                              l1_cost_volume_predict)
                ...
                if itr == 0 :
                    ...
                    l1_points_f1_new_big = self.conv3_list_l1[itr-1](l1_cost_volume_sum)
                    # l1_points_f1_new_big = self.conv3_1(l1_cost_volume_sum)
                ...
                if itr == 0 :
                    ...
                    l1_q_det = self.conv1_list_l1[itr-1](l1_points_f1_new_q)
                    # l1_q_det = self.conv1_1(l1_points_f1_new_q)
                ...
                if itr == 0 :
                    ...
                    l1_t_det = self.conv2_list_l1[itr-1](l1_points_f1_new_t)
                    # l1_t_det = self.conv2_1(l1_points_f1_new_t)
                ...
                pre_qt.append([l1_q_norm, l1_t])


            ################# layer0 PWC ###################


            l0_q_coarse = torch.reshape(q_now, [batch_size, 1, -1])
            l0_t_coarse = torch.reshape(t_now, [batch_size, 1, -1])
            l0_q_inv = inv_q(l0_q_coarse, batch_size)

            ############# warp layer0 pose

            l0_xyz_f1 = torch.reshape(l0_xyz_proj_f1, [batch_size, -1, 3]).clone()
            ...
            ########## re-project
            l0_xyz_warp_proj_f1, l0_points_warp_proj_f1 = ProjectPCimg2SphericalRing(l0_flow_warped, l0_points_f1,
                                                                                     self.out_H_list[0],
                                                                                     self.out_W_list[0])  #
            ...
            l0_mask_warped_proj = torch.reshape(l0_mask_warped,
                                                [batch_size, self.out_H_list[0], self.out_W_list[0], -1])

            # get the cost volume of warped layer0 flow and the points of frame2
            l0_points_warp_cross_f1, l0_points_warp_cross_f2 = self.cross_trans0(l0_points_warp_f1, l0_points_f2,
                                                                                 l0_mask_warped_proj, l0_mask_f2)
            l0_points_warp_cross_proj_f1 = torch.reshape(l0_points_warp_cross_f1,
                                                         [batch_size, self.out_H_list[0], self.out_W_list[0], -1])
            l0_points_warp_cross_proj_f2 = torch.reshape(l0_points_warp_cross_f2,
                                                         [batch_size, self.out_H_list[0], self.out_W_list[0], -1])
            
            if itr == 0 :
                l0_cost_volume = self.cost_volume4(l0_xyz_warp_proj_f1, l0_xyz_proj_f2, l0_points_warp_cross_proj_f1,
                                               l0_points_warp_cross_proj_f2)  # FE0
            else:
                l0_cost_volume = self.cost_list_volume4[itr - 1](l0_xyz_warp_proj_f1, l0_xyz_proj_f2, l0_points_warp_cross_proj_f1,
                                               l0_points_warp_cross_proj_f2)
            # l0_cost_volume = self.cost_volume4(l0_xyz_warp_proj_f1, l0_xyz_proj_f2, l0_points_warp_cross_proj_f1,
            #                                    l0_points_warp_cross_proj_f2)
            if itr == 0 :
                ...
                l0_cost_volume_w_upsample = self.set_upconv3_list_w_upsample[itr - 1](l0_xyz_warp_proj_f1, l1_xyz_warp_proj_f1,
                                                                        l0_points_warp_proj_f1, l1_cost_volume_w_proj)
            # elif itr == 2:
            #     l0_cost_volume_w_upsample = self.set_upconv3_list_w_upsample[itr - 1](l0_xyz_warp_proj_f1,l0_xyz_warp_proj_f1,
            #                                                                           l0_points_warp_proj_f1,l0_cost_volume_w_proj)
            if itr == 0:
                ...
                l0_cost_volume_upsample = self.set_upconv3_list_upsample[itr - 1](l0_xyz_warp_proj_f1, l1_xyz_warp_proj_f1,
                                                                    l0_points_warp_proj_f1, l1_cost_volume_proj)
            # elif itr == 2:
            #     l0_cost_volume_upsample = self.set_upconv3_list_upsample[itr - 1](l0_xyz_warp_proj_f1, l0_xyz_warp_proj_f1,
            #                                                                       l0_points_warp_proj_f1, l0_cost_volume_proj)
            ...
            if itr == 0:
                ...
                l0_cost_volume_predict = self.flow_predictor3_list_predict[itr - 1](l0_points_warp_f1, l0_cost_volume_upsample,
                                                                      l0_cost_volume)
            if itr == 0:
                l0_cost_volume_w = self.flow_predictor3_w(l0_points_warp_f1, l0_cost_volume_w_upsample,
                                                      l0_cost_volume_predict)
            else:
                l0_cost_volume_w = self.flow_predictor3_list_w[itr - 1](l0_points_warp_f1, l0_cost_volume_w_upsample,
                                                          l0_cost_volume_predict)

            l0_cost_volume_proj = torch.reshape(l0_cost_volume_predict,
                                                [batch_size, self.out_H_list[0], self.out_W_list[0], -1])
            l0_cost_volume_w_proj = torch.reshape(l0_cost_volume_w,
                                                  [batch_size, self.out_H_list[0], self.out_W_list[0], -1])
            ...
            if itr == 0:
                ...
                l0_points_f1_new_big = self.conv3_list_l0[itr - 1](l0_cost_volume_sum)
                # l0_points_f1_new_big = self.conv3_0(l0_cost_volume_sum)
            ...
            if itr == 0:
                ...
                l0_q_det = self.conv1_list_l0[itr - 1](l0_points_f1_new_q)
                # l0_q_det = self.conv1_0(l0_points_f1_new_q)
            ...
            if itr == 0:
                ...
                l0_t_det = self.conv2_list_l0[itr - 1](l0_points_f1_new_t)
                # l0_t_det = self.conv2_0(l0_points_f1_new_t)
        ...
        return pre_qt, q_gt, t_gt, self.w_x, self.w_q
...
def get_loss(qt_list, qq_gt, t_gt, w_x, w_q):

        t_gt = torch.squeeze(t_gt)
        d = 1.6
        for i,[l0_q,l0_t] in enumerate(qt_list):
            # if i < 4:
            #     continue
            l0_q_norm = l0_q / (torch.sqrt(torch.sum(l0_q * l0_q, dim=-1, keepdim=True) + 1e-10) + 1e-10)
